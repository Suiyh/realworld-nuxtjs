		...
		<div class="row">
				...
				<div class="feed-toggle">
					<ul class="nav nav-pills outline-active">
						...
						<li v-if="tag" class="nav-item">
							<nuxt-link class="nav-link" :class="{ active: tab === tag }" :to="{ name: 'home', query: { tab: 'tag', tag: tag } }" exact>#{{ tag }}</nuxt-link>
						</li>
					</ul>
				</div>
				...
				<nav>
					<ul class="pagination">
						<li class="page-item" :class="{ active: item === page }" v-for="item in totalPage" :key="item">
							<nuxt-link class="page-link" :to="{ name: 'home', query: { page: item, tag: $route.query.tag, tab: tag } }">{{item}}</nuxt-link>
						</li>
					</ul>
				</nav>
			...
			<div class="col-md-3">
				<div class="sidebar">
					<p>Popular Tags</p>
					<div class="tag-list">
						<nuxt-link v-for="tag in tags" :key="tag" :to="{ name: 'home', query: { tab: 'tag', tag: tag }}" class="tag-pill tag-default">{{tag}}</nuxt-link>
					</div>
				</div>
			</div>
